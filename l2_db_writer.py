...
def write_to_influxdb(data: dict) -> None:
    ...
    try:                                                                  
        point = (                                                         
            ...
            .field("humidity", float(data["humidity"]))               
            .field("pressure", float(data["pressure"]))               
            .field("co2", float(data["co2_ppm"]))                    
            .field("aqi", float(data["aqi"]))
            .field("light_level", float(data["light_lux"]))        
            .field("wind_speed", float(data["wind_speed"]))
            .field("wind_dir", float(data["wind_direction"]))         
            .field("rainfall_mm", float(data["rainfall_mm"]))           
            .field("battery_pct", float(data["battery_pct"]))    
            .field("rssi", int(data["rssi"]))                      
           )                                                                 
        _write_api.write(bucket=INFLUX_BUCKET, record=point)
        all_metrics = "  ".join([f"{k}={v}" for k, v in data.items() if k not in ["device_id", "timestamp"]])
        print(f"[INFO] [INFLUX_WRITE] timestamp={data['timestamp']} device_id={data['device_id']} {all_metrics}")             
        #print(f"[INFLUX] Written: {data['device_id']}  "f"temp={data['temperature']}  co2={data['co2']}")#          
    except KeyError as e:
        print(f"[ERROR] [INFLUX_WRITE] Missing expected sensor key in payload: {e}")
    except Exception as e:                                               
        print(f"[CRITICAL] [INFLUX_WRITE] Write failed: {e}")                              
